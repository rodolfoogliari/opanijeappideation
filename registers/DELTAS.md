...
| D94 | The play layer ships ON for students | `playLayerEnabled` (`AppRuntime.tsx:239-242`) false unless `__DEV__` or `EXPO_PUBLIC_ROOM_DEMO`; `closing.test.tsx:234` actively asserts the three facts are absent by default | *Proposed:* true in production builds — the echo loop, the fade-and-rejoin model and the three-facts-plus-personal-best closing screen become visible to students. Three consequences, none optional: `closing.test.tsx:234` inverts and every test written against the play layer's absence flips with it; Junior must see it before it is public (Charter §9 item 10 — the play layer *is* screen-drum surface); and red line #5 makes it permanent for the free tier | **PENDING** — founder held 2026-08-05 for a fuller account before ruling. Nothing in the build proceeds against it either way. Highest-priority open decision in the estate, per row 59 | ONE-WAY — red line #5 attaches | PLAN-00 §3; BUILD-LOG 2026-08-05 |
...
**Three rows on this page are not ADOPTED and must not be read as though they were.** D94 is
**PENDING** and nothing may be built against it in either direction. D96 is **REJECTED** — any
...
ratification. Numbering state: last used **D103**, next **D104** — but under D88 no D104 will be

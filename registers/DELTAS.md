...
| D84 | Monthly master cycle out of Release 1 | D81: the cycle is Release 1 operations, membership sold day one | Deferred out of R1. Junior's monthly hours go to Track B private classes, which produce revenue evidence. INPUT-82's go/no-go answered **NO-GO for now**; the cycle returns when 100 weekly actives exist to attend it, and returns as the Perguntas library (PLAN-01 Stage 5, Concept C) rather than a live event | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §1; BUILD-LOG 2026-08-05 |
| D85 | The notation standard is frozen, not cancelled | D82 / G22: publish the notation system openly as a standard | The frame stays drafted and in the drawer. Risk #28 is not a risk at corpus size zero, it is a certainty. Revisited when there is a corpus for it not to outrun. G22's positioning claim is untouched — Opanijé can position as a music-education company without having published anything | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §1 |
| D86 | A lean Capture Day replaces the full M0 production shoot for Release 1 | M0 as a full production shoot (v2.0 §7.2) | One kitchen-table capture day. **The four one-way parts are not cut and not softened:** consent scope covering interactive and game use signed before anything rolls (INPUT-22/-89), isolated per-part stems (INPUT-23), the partition captured item-by-item as Junior speaks it (INPUT-21), and solfejo passes locked to the battery. A lean day that skips any of the four is not a lean day, it is a wasted one | RATIFIED 2026-08-05 | **Partly ONE-WAY** — the estate's own logic makes M0 governing by *irrecoverability*: what is shot is one-way, when the big shoot happens is not | PLAN-00 §1 |
| D87 | The app sells nothing and links to no checkout | Commerce reachable from the app | All commerce on the web through the already-deployed Mercado Pago rail. The app asks the server what an account owns and plays it. Deletes Google Play billing, the 15–30% store tax and the anti-steering compliance brief from the critical path in one move, and makes the Play listing trivially compliant | RATIFIED 2026-08-05 | TWO-WAY — but not to be reversed casually | PLAN-00 §1 |
| D88 | The register apparatus goes to maintenance mode | Nine registers, all live and extended each session | `CHARTER.md`, the vocabulary sheet (METHOD §5), the Charter §9 prohibition list and `LEDGER.md` stay alive. `DELTAS.md`, `RECOMMENDATIONS.md`, `CONTRADICTIONS.md`, `FORKS.md` and `sessions/` freeze: read-only, cited when useful, never extended. Build decisions become one line in `BUILD-LOG.md`. **Amendment:** Room's `GATES.md` freezes as a document only *after* its 56 in-code `GATE:` markers are swept once (`check-gates.mjs`) and each closed, converted to a BUILD-LOG line, or deferred with a date — freezing the register while the markers stay in shipping code creates orphans | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §1; BUILD-LOG 2026-08-05 |
| D89 | `apps/opanije-room` is the product | Room described as a mockup | All future work lands here. It is the only artifact on earth implementing the estate's actual inventions: 12,443 lines of implementation, 621 tests across 55 suites, 13 bilingual routes. It stops being described as a mockup | RATIFIED 2026-08-05 | ONE-WAY in practice | PLAN-00 §2; VERIFICATION Claim 1 |
| D90 | `apps/opanije-mobile` is a parts donor, then archived | A second app under active consideration | Four modules lifted into Room — the hand-rolled PKCE auth flow, `expo-secure-store` session handling, the repository factory, and the `expo-video` signed-playback pattern — then archived with a README pointer, **not deleted**, because its iOS project is the estate's only iOS asset. Treated as **porting from a reference implementation, not moving working code**: formally PARKED under DN-3, CI reduced to `workflow_dispatch` after 30 consecutive typecheck failures, `node_modules` absent, current state unverified. Two-day timebox per module, then write fresh against Room's conventions | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §2; VERIFICATION Claim 9 |
| D91 | The dormant mu-plugin rail is the backend, forever absent a founder ruling | Backend undecided | 13 opanije mu-plugins, ~360 KB, ~9,600 lines, deployed and versioned with the `opanije-mobile/v1` REST contract, shared fixtures and a JSON schema. Nobody proposes a new backend. Every server-side feature is a new endpoint in that contract. The VPS runs it today at 0.04 load | RATIFIED 2026-08-05 | ONE-WAY | PLAN-00 §2; VERIFICATION Claims 11–12 |
| D92 | Production bundle ID is `com.opanije.room` | `com.opanije.room.mockup` (Room); `com.opanije.course.debug.preview` (parked) | Verified free. Renamed **before the first store upload, never after** — changing it post-publication means a new listing and orphaned installs. The rename also touches the keychain service string (orphaning any stored sessions) and deep-link configuration | RATIFIED 2026-08-05 | ONE-WAY once published | PLAN-00 §2; VERIFICATION Claim 14 |
| D93 | Room's outward seams stay unwired until Stage 4 | Seams assumed near-complete; 3–4 weeks estimated | `createRoomApi` is never called, `createCommercePlanner` is imported by no screen, `RemoteRenderSource` is never instantiated, `AccountIdentityPort` has no implementation, `factSender` throws by default, `catalogForPublishing()` throws unconditionally, and `SERVER-CONTRACT.md:5` calls its endpoints *"proposed shapes for the missing implementation."* Connecting Room to the server is **greenfield behind a designed interface, not a wiring job**. Budget 5–7 vibecoding weeks, scheduled after the free instrument has shipped and shown pull | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §2; VERIFICATION Claim 10 |
| D94 | The play layer ships ON for students | `playLayerEnabled` (`AppRuntime.tsx:239-242`) false unless `__DEV__` or `EXPO_PUBLIC_ROOM_DEMO`; `closing.test.tsx:234` actively asserts the three facts are absent by default | Split into **D94a** (the play layer in tester builds) and **D94b** (the play layer in the public free room). The decision as issued bundled a reversible commitment with a one-way one; the founder ratified the reversible half and scheduled the other. Split rather than renumbered, on the estate's own precedent for ledger row 21 (21a/21b by D12) | **SPLIT 2026-08-05** — see the two rows below | **SPLIT** — cite D94a or D94b, never bare D94 | PLAN-00 §3; BUILD-LOG 2026-08-05 |
| D94a | The play layer ships ON in **tester builds** | Never seen by anyone outside the estate — every build a human has received had it off, including the device test that produced row 59 | ON for Junior's Day 10 sitting and for Stage 2's ten testers. **Red line #5 does not attach**: a hand-picked sideload is not the free tier, and D103 already ruled that same cohort is not "public". Rows 52, 53 and 55 and Junior's form assent (INPUT-79) are all collected from this build. **Scope is a bounded code change, not zero** — `EXPO_PUBLIC_ROOM_DEMO` currently gates the play layer *and* the demonstration route tree (`REVIEW_ROUTE_TREE = ['review', 'past-the-door']`, `_layout.tsx:48`), and `build-room-apk.sh:63-64` requires those surfaces stay off in anything a student receives, so the two gates are separated first | RATIFIED 2026-08-05 | TWO-WAY | BUILD-LOG 2026-08-05; VERIFICATION Claim 3b/3d |
| D94b | The play layer default flips ON in the **public free room** | — | Taken at **Stage 3**, with row 52 (Junior's verdict), row 55 (≥7/10) and the ten testers' answers in hand. Carries the `closing.test.tsx:234` inversion and every test written against the play layer's absence. Charter §9 item 10 is discharged in advance by the Day 10 sitting under D94a, so the master's assent exists before this is put | **PENDING — scheduled for Stage 3** | **ONE-WAY** — red line #5 attaches: the free room keeps the echo loop, the fade and the closing facts permanently | BUILD-LOG 2026-08-05 |
| D95 | The fade model stays binary for Release 1 | The estate's fade read as a decay model with cross-session memory | Shipped as built: binary `full` (0.6) / `thin` (0.2), recomputed every authored cycle, applied to the selected-part companion only, recovering fully the next cycle. **No decay curve and no cross-session unfed decay** — nothing records a last-practice timestamp. Satisfies D78 (no fail state) and D77 (the drum always sounds), and is already tested. The real feeding measure at `tap.ts:82-83` is computed and then discarded; it is wired to the ledger as telemetry and not acted on. Whether a curve is even desirable is what Stage 2's ten humans are for | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §3; VERIFICATION Claim 3c; row 53 |
| D96 | The web room is the iOS strategy and the primary distribution surface | — (proposed at PLAN-00 §3) | **Not adopted.** The web room is neither the iOS strategy nor the primary distribution surface. Release 1 consequently carries **no iOS path** and **no no-install tap-to-play surface**, and what Junior's voice note points at is reopened at INPUT-90. The web export itself is not prohibited by this ruling — only its promotion to strategy | **REJECTED 2026-08-05** — founder ruling. No reason recorded and none is owed | TWO-WAY | PLAN-00 §3; BUILD-LOG 2026-08-05; risk #36; INPUT-90 |
| D97 | Purchases are per-course from day one | Fable's constraint: one product only until a `W1-CL-ENTITLE` per-course-grant defect is fixed | Per-course from day one. **The defect does not exist at HEAD.** `opanije_course_access_subject_holds_course()` (`opanije-course-access.php:1511-1546`) keys on `course_key` in a two-level ledger and re-verifies the stored key; `opanije_course_app_user_may_read()` routes through it; the product map is a map, not a scalar; `OPANIJE_COURSE_ACCESS_MAX_COURSES = 2` and the tests already exercise two course keys. It existed at `4252113d` and was closed by `94a2a0dd` and `694da476`. **Remediation scheduled against that claim is wasted work — do not do it.** Confirm with one live test purchase at activation | RATIFIED 2026-08-05 | TWO-WAY | PLAN-00 §3; VERIFICATION "Claims that are wrong" §1 |
| D98 | A real release keystore is created and escrowed before any store upload | One machine holds the code, every credential, the backup key and the signing key with no standby (D16, the estate's highest-ranked overdue item) | *Proposed:* a production keystore generated once, escrowed to two off-box locations with a restored passphrase and a verified test restore, never printed to a log, merged into a single escrow event with D16 | **DEFERRED 2026-08-05** — founder ruling. Not cancelled. Two consequences follow mechanically: **no store upload is possible until it closes**, and the single-machine total-loss exposure **stays open**. `PLAN-02-RUNBOOK.md` Day 1 amended | ONE-WAY | PLAN-00 §4; BUILD-LOG 2026-08-05; risk #37; INPUT-91 |
| D99 | Two physical Android handsets before public ship | `ACCEPTANCE.md:29` criterion 1 BLOCKED, owner-assigned to the operator, *"NOT VERIFIED ON A DEVICE"* | The founder's phone plus one used low-tier Android, ~R$400 — the single hardware purchase these plans endorse. The two things only a human can verify are how the audio *feels* through a speaker and how touch latency *feels* under a finger. Emulator evidence is not device evidence. **Partly executed as of ratification:** a device test has been run and returned 0/10 — see row 59 and risk #35 | RATIFIED 2026-08-05 | TWO-WAY, not skippable | PLAN-00 §4; BUILD-LOG 2026-08-05 |
...
## The D84–D103 block — what it is, and why it is the last
...
product decision; the Perguntas library is Concept C in `PLAN-01-BUILD.md` Stage 5 and carries no
D-number. The row above cites the correct source. The slip is noted here because a quiet renumbering
is how a register loses its authority (`NUMBERING.md`).

**Three rows on this page are not ADOPTED and must not be read as though they were.** D94 was
**SPLIT** on 2026-08-05: **D94a is RATIFIED** (the play layer in tester builds, two-way) and
**D94b is PENDING**, scheduled for Stage 3 (the public free room, one-way under red line #5).
**Never cite bare "D94"** — it names a decision that no longer exists as a unit, exactly as bare
"row 21" does not exist after D12. D96 is **REJECTED** — any
downstream document that still treats the web room as the iOS strategy is stale, and
`PLAN-01`/`PLAN-02`/`PLAN-03` were amended on 2026-08-05 to match. D98 is **DEFERRED**, which is not
cancellation: the work is still required before any store upload.
